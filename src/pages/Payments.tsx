...
const Payments = () => {
  ...
  const onSubmit = (data: PaymentForm) => {
    ...
    toast.success("Payment initiated", {
      description: `${data.type} payment of ${formatCurrency(data.amount, data.currency)} submitted.`,
    });
    reset();
    setOpen(false);
    ...
    setTimeout(() => {
      setTxns((prev) =>
        prev.map((t) =>
          t.id === newTx.id ? { ...t, status: Math.random() > 0.2 ? "success" : "failed" } : t,
        ),
      );
  ...
  const filtered = txns.filter((t) => {
    ...
    if (
      search &&
      !`${t.sender} ${t.receiver} ${t.reference}`.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
    ...
    <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    defaultValue="ACH"
                    onValueChange={(v) => setValue("type", v as PaymentType)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    defaultValue="USD"
                    onValueChange={(v) => setValue("currency", v as "USD" | "AED")}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div>
                ...
                {errors.sender && (
                  <p className="text-destructive text-xs mt-1">{errors.sender.message}</p>
                )}
              </div>
              <div>
                ...
                {errors.receiver && (
                  <p className="text-destructive text-xs mt-1">{errors.receiver.message}</p>
                )}
              </div>
              <div>
                <Label>Amount</Label>
      ...
      >
        ...
        <Select
          value={typeFilter}
          onValueChange={(v) => {
            setTypeFilter(v as PaymentType | "all");
            setPage(1);
          }}
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/50">
                {["Reference", "Type", "Sender", "Receiver", "Amount", "Date", "Status"].map(
                  (h) => (
                    <th
                      key={h}
